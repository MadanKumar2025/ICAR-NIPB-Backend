import TermsConditions from "../models/TermsConditionsSchema.js";
...
export const createTermsConditions = async (req, res) => {
  try {
    ...
    await TermsConditions.updateMany({}, { isActive: false });

    const newData = new TermsConditions({
      content: {
        en: content_en.trim(),
        hi: content_hi.trim(),
      },
...
export const getAllTermsConditions = async (req, res) => {
  try {
    ...
    let query = TermsConditions.find(filter)
      .populate("createby", "username email")
      .populate("updateby", "username email")
      .sort({ createdate: -1 });

    const total = await TermsConditions.countDocuments(filter);
    ...
    console.error("Get TermsConditions Error:", error);
...
export const updateTermsConditions = async (req, res) => {
  try {
    ...
    if (content_en) {
      terms.content.en = content_en.trim();
    }

    if (content_hi) {
      terms.content.hi = content_hi.trim();
    }
    ...
    if (isActive !== undefined) {
      ...
      terms.isActive = activeValue;
    }

    terms.updateby = req.user?.id;
    terms.updatedate = new Date();

    const updatedData = await terms.save();
    ...
    console.error("Update TermsConditions Error:", error);
...
// this is use for web
export const getAllTermsConditionsWeb = async (req, res) => {
  try {
