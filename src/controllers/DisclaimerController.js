...
export const createDisclaimer = async (req, res) => {
  try {
    ...
    const savedDisclaimer = await disclaimer.save();

    return res.status(201).json({
      success: true,
      message: "Disclaimer created successfully",
      data: savedDisclaimer,
    });
  } catch (error) {
...
export const getAllDisclaimers = async (req, res) => {
  try {
    ...
    let query = Disclaimer.find(filter)
      .populate("createby", "username")
      .populate("updateby", "username")
      .sort({ createdate: -1 });

    const totalCount = await Disclaimer.countDocuments(filter);
    ...
    console.error("Get Disclaimers Error:", error);
...
export const updateDisclaimer = async (req, res) => {
  try {
    ...
    if (content_en) {
      disclaimer.content.en = content_en;
    }

    if (content_hi) {
      disclaimer.content.hi = content_hi;
    }
    ...
    disclaimer.updateby = req.user?.id;
    disclaimer.updatedate = new Date();

    const updatedDisclaimer = await disclaimer.save();
...
// this is use for web
export const getAllDisclaimerWeb = async (req, res) => {
  try {
    const disclaimerList = await Disclaimer.find()
      .populate("createby", "name email")
      .populate("updateby", "name email")
      .sort({ createdate: -1 });

    const data = disclaimerList.map((item) => ({
      id: item._id,
